...
export class UbaSmsParser {
  ...
  // Strict Disambiguation: Amt vs Bal
  ...
  private static BALANCE_PATTERN = /(?:Bal|Balance|Avail\s+Bal|Ledger\s+Bal|New\s+Bal)\s*:?\s*(?:NGN|₦)?\s*([0-9,]+\.[0-9]{2})/i;
  private static AMOUNT_PATTERN = /(?:Amt|Amount|Txn\s*Amt)\s*:?\s*(?:NGN|₦)?\s*([0-9,]+\.[0-9]{2})/i;
  private static ACCT_PATTERN = /Acc(?:t|:)?\s*[:\s]*[\d\*X\.]*(\d{4})/i;
